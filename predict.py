"""
Cog predictor for Apple Sharp model.

Converts a single image to 3D Gaussian splats (PLY format).
Based on https://github.com/apple/ml-sharp and the SHARP-ML Modal deployment.
"""
...
# The original Sharp code moves tensors to CPU for SVD and uses scipy for
# quaternion conversion. These functions keep everything on GPU for ~10x speedup.
# =============================================================================
...
def fast_apply_transform_gpu(gaussians, transform: torch.Tensor):
...
def fast_save_ply_bytes(gaussians, f_px: float, image_shape: tuple) -> bytes:
    ...
    Minimizes GPU->CPU transfers and uses efficient numpy structured arrays.
    """
    ...
    with torch.no_grad():
        ...
        quantiles = torch.quantile(
            disparity, q=torch.tensor([0.1, 0.9])
        ).numpy()
    ...
    # Build structured array directly
    dtype_full = np.dtype([
        ('x', 'f4'), ('y', 'f4'), ('z', 'f4'),
    ...
    frame_arr = np.array(
        [(1,), (num_gaussians,)], dtype=[('frame', 'i4')]
    )
    ...
    version_arr = np.array(
        [(1,), (5,), (0,)], dtype=[('version', 'u1')]
    )
...
class SharpPredictor(BasePredictor):
    """Cog predictor for Apple Sharp: single image to 3D Gaussian splats."""

    def setup(self):
        """Download the Sharp checkpoint, create predictor, load weights, warmup."""
        ...
        # Warmup: run a dummy forward pass to compile CUDA kernels
    ...
    def predict(
        self,
        image: CogPath = Input(description="Input image (single view)"),
    ) -> CogPath:
        """Convert a single image to 3D Gaussian splats (PLY file)."""
        ...
        # Preprocess: convert to tensor, resize to internal resolution
        image_pt = (
        ...
        disparity_factor = torch.tensor(
            [f_px / width], device=self.device
        ).float()

        image_resized = F.interpolate(
            image_pt[None],
            size=(INTERNAL_RESOLUTION[1], INTERNAL_RESOLUTION[0]),
            mode="bilinear",
            align_corners=True,
        )
        ...
        # Postprocess: unproject from NDC to metric space
        ...
        intrinsics_resized[0] *= INTERNAL_RESOLUTION[0] / width
        intrinsics_resized[1] *= INTERNAL_RESOLUTION[1] / height
